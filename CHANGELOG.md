...
- DualSense-aware Dashboard labels with native PS5 symbols and live physical-button feedback.
- Bounded normalized controller event history for local HUD diagnostics.

### Changed

- Dashboard redesigned as a light monochrome controller console.
- Project website moved to `https://codingmacro.vercel.app`.
...
- GitHub release tarballs now run `npm run build` through `prepack`, so fresh CI artifacts include `dist/` and the `codingmacro` executable.
- Installer verifies the executable through npm's global prefix instead of assuming that prefix is already present in `PATH`.
