...
当前优先支持 macOS，要求 Node.js 22+。安装器会安装 CLI，并从 `~/Applications/CodingMacro.app` 打开菜单栏应用。只有点击 **Enable Accessibility…** 后才会请求辅助功能权限。

## 启动

菜单栏可直接启动 Codex、模拟器、Dashboard 和手柄 Doctor。终端命令仍可使用：
