...
When a wrapped session runs inside a [herdr](https://github.com/stephenleo/herdr)-managed pane, OpenMicro also reports its state (working/blocked/idle) to herdr, so sessions show up in the herdr workspace overview. This is automatic and a no-op outside herdr.
